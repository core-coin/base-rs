...
/// is done in the [`crate::YlmType`] trait.
#[derive(Clone, Copy)]
...
/// [`YlmValue::abi_decode`](crate::YlmValue::abi_decode) if you are not
...
///
/// You are probably looking for
/// [`YlmValue::abi_decode_params`](crate::YlmValue::abi_decode_params) if
...
///
/// You are probably looking for
/// [`YlmValue::abi_decode_sequence`](crate::YlmValue::abi_decode_sequence) if
/// you are not intending to use raw tokens.
///
/// See the [`abi`](super) module for more information.
#[inline]
...
#[cfg(test)]
mod tests {
    use crate::{utils::pad_usize, ylm, ylm_data, YlmType, YlmValue};
    use alloc::string::ToString;
    use base_primitives::{address, bytes, cAddress, hex, IcanAddress, B256, U256};
    ...
    fn decode_static_tuple_of_addresses_and_uints() {
        type MyTy = (ylm_data::Address, ylm_data::Address, ylm_data::Uint<256>);
    ...
    fn decode_after_fixed_bytes_with_less_than_32_bytes() {
        type MyTy = (
            ylm_data::Address,
            ylm_data::FixedBytes<32>,
            ylm_data::FixedBytes<4>,
            ylm_data::String,
        );
    ...
    fn decode_broken_utf8() {
        ...
        assert_eq!(ylm_data::String::abi_decode(&encoded, false).unwrap(), "不�".to_string());
    }

    #[test]
    fn decode_corrupted_dynamic_array() {
        type MyTy = ylm_data::Array<ylm_data::Uint<32>>;
    ...
    fn decode_verify_addresses() {
        ...
        assert!(ylm_data::Address::abi_decode(&input, false).is_ok());
        assert!(ylm_data::Address::abi_decode(&input, true).is_err());
        assert!(<(ylm_data::Address, ylm_data::Address)>::abi_decode(&input, true).is_ok());
    }

    #[test]
    fn decode_verify_bytes() {
        type MyTy = (ylm_data::Address, ylm_data::FixedBytes<22>);
        type MyTy2 = (ylm_data::Address, ylm_data::Address);
    ...
    // https://github.com/core-coin/base-rs/issues/433
    #[test]
    fn fixed_before_dynamic() {
        ...
        assert_eq!(<Ty as YlmType>::abi_decode(&encoded, true).unwrap(), ty);
    }
    ...
    fn dynarray_before_dynamic() {
        ...
        assert_eq!(<Ty as YlmType>::abi_decode(&encoded, false).unwrap(), ty);
    }
}
