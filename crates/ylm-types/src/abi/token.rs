...
use alloc::vec::Vec;
use base_primitives::{utils::vec_try_with_capacity, Bytes, FixedBytes, I256, U256};
use core::fmt;
...
impl<'de, T: Token<'de>> Token<'de> for DynSeqToken<T> {
    ...
    fn decode_from(dec: &mut Decoder<'de>) -> Result<Self> {
        ...
        // This appears to be an unclarity in the Ylem spec. The spec
...
mod tests {
    ...
    fn test_type_check() {
        assert_type_check!(
            (ylm_data::Uint<256>, ylm_data::Bool),
            &(WordToken(B256::default()), WordToken(B256::default())),
        );

        // TODO(tests): more like this where we test type check internal logic
        assert_not_type_check!(ylm_data::Uint<8>, &Word::repeat_byte(0x11).into());
        assert_not_type_check!(ylm_data::Bool, &B256::repeat_byte(0x11).into());
        assert_not_type_check!(ylm_data::FixedBytes<31>, &B256::repeat_byte(0x11).into());

        assert_type_check!(
            (ylm_data::Uint<32>, ylm_data::Bool),
            &(WordToken(B256::default()), WordToken(B256::default())),
        );

        assert_type_check!(
            ylm_data::Array<ylm_data::Bool>,
        ...
        assert_type_check!(
            ylm_data::Array<ylm_data::Address>,
            &DynSeqToken(vec![WordToken(B256::default()), WordToken(B256::default()),]),
        );

        assert_type_check!(
            ylm_data::FixedArray<ylm_data::Bool, 2>,
            &FixedSeqToken::<_, 2>([
        ...
        assert_type_check!(
            ylm_data::FixedArray<ylm_data::Address, 2>,
            &FixedSeqToken::<_, 2>([
                WordToken(B256::default()),
                WordToken(B256::default()),
            ]),
