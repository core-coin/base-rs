use crate::{alloc::string::ToString, Error, Panic, Result, Revert, YlmError};
...
///
/// [`YlmCall`]: crate::YlmCall
/// [`YlmError`]: crate::YlmError
///
/// # Implementer's Guide
///
...
/// An empty [`YlmInterface`] implementation. Used by [`GenericContractError`].
impl YlmInterface for Infallible {
...
impl<T: YlmInterface> From<T> for ContractError<T> {
    #[inline]
...
impl<T: YlmInterface> YlmInterface for ContractError<T> {
...
impl<T: YlmInterface> RevertReason<T>
where
...
impl<T: YlmInterface + fmt::Display> RevertReason<T> {
...
/// Iterator over the function or error selectors of a [`YlmInterface`] type.
///
/// This `struct` is created by the [`selectors`] method on [`YlmInterface`].
/// See its documentation for more.
///
/// [`selectors`]: YlmInterface::selectors
...
impl<T: YlmInterface> Iterator for Selectors<T> {
    type Item = [u8; 4];
...
impl<T: YlmInterface> ExactSizeIterator for Selectors<T> {
    #[inline]
    fn len(&self) -> usize {
        T::COUNT - self.index
    }
...
impl<T: YlmInterface> FusedIterator for Selectors<T> {}

#[cfg(test)]
mod tests {
    use super::*;
    use base_primitives::{sha3, U256};
