use crate::{abi::token::WordToken, ylm_data::*, YlmType};
use alloc::vec::Vec;
use base_primitives::sha3;

/// A Ylem event topic.
///
/// These types implement a special encoding used only in Ylem indexed event
/// parameters.
///
...
pub trait EventTopic: YlmType {
    ...
    ///
    /// See the [Ylem ABI spec][ref] for more details.
    ///
    /// [ref]: https://docs.soliditylang.org/en/latest/abi-spec.html#encoding-of-indexed-event-parameters
    ...
    /// [`YlmType::abi_encode`]. See the [Ylem ABI spec][ref] for more
    /// details.
    ///
    /// [`encode_topic_preimage`]: EventTopic::encode_topic_preimage
    /// [ref]: https://docs.soliditylang.org/en/latest/abi-spec.html#encoding-of-indexed-event-parameters
...
macro_rules! word_impl {
    () => {
        ...
        #[inline]
        fn encode_topic_preimage(rust: &Self::RustType, out: &mut Vec<u8>) {
            out.extend($crate::private::YlmTypeValue::<Self>::stv_to_tokens(rust).0);
        }

        #[inline]
        fn encode_topic(rust: &Self::RustType) -> WordToken {
            $crate::private::YlmTypeValue::<Self>::stv_to_tokens(rust)
        }
    };
}
