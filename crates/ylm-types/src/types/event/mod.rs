...
use alloc::vec::Vec;
use base_primitives::{FixedBytes, Log, LogData, B256};
...
pub trait YlmEvent: Sized {
    ...
    /// `()`.
    type DataTuple<'a>: YlmType<Token<'a> = Self::DataToken<'a>>;
    ...
    fn new(
        topics: <Self::TopicList as YlmType>::RustType,
        data: <Self::DataTuple<'_> as YlmType>::RustType,
    ) -> Self;
    ...
    /// The event's topics.
    fn topics(&self) -> <Self::TopicList as YlmType>::RustType;

    /// The size of the ABI-encoded dynamic data in bytes.
    #[inline]
    fn abi_encoded_size(&self) -> usize {
        if let Some(size) = <Self::DataTuple<'_> as YlmType>::ENCODED_SIZE {
            return size;
        }
    ...
    #[inline]
    fn decode_topics<I, D>(topics: I) -> Result<<Self::TopicList as YlmType>::RustType>
    where
    ...
    fn abi_decode_data<'a>(
        data: &'a [u8],
        validate: bool,
    ) -> Result<<Self::DataTuple<'a> as YlmType>::RustType> {
        <Self::DataTuple<'a> as YlmType>::abi_decode_sequence(data, validate)
    }
