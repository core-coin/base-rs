//! Ylem types.
//!
//! These are the types that are [built into Ylem][ref].
//!
//! See [`YlmType`] for more details.
//!
//! [ref]: https://docs.soliditylang.org/en/latest/types.html

#![allow(missing_copy_implementations, missing_debug_implementations)]

use crate::{abi::token::*, private::YlmTypeValue, utils, Word, YlmType};
...
// `ylm-macro/src/expand/ty.rs`

/// Bool - `bool`
pub struct Bool;

impl YlmTypeValue<Bool> for bool {
    #[inline]
    ...
    fn stv_eip712_data_word(&self) -> Word {
        YlmTypeValue::<Bool>::stv_to_tokens(self).0
    }
}

impl YlmType for Bool {
    type RustType = bool;
    type Token<'a> = WordToken;

    const YLM_NAME: &'static str = "bool";
...
impl<T, const BITS: usize> YlmTypeValue<Int<BITS>> for T
where
    ...
    IntBitCount<BITS>: SupportedInt,
{
    ...
    #[inline]
    fn stv_eip712_data_word(&self) -> Word {
        YlmTypeValue::<Int<BITS>>::stv_to_tokens(self).0
    }
}

impl<const BITS: usize> YlmType for Int<BITS>
where
    IntBitCount<BITS>: SupportedInt,
{
    ...
    const YLM_NAME: &'static str = IntBitCount::<BITS>::INT_NAME;
    const ENCODED_SIZE: Option<usize> = Some(32);
...
impl<const BITS: usize, T> YlmTypeValue<Uint<BITS>> for T
where
    ...
    IntBitCount<BITS>: SupportedInt,
{
    ...
    #[inline]
    fn stv_eip712_data_word(&self) -> Word {
        YlmTypeValue::<Uint<BITS>>::stv_to_tokens(self).0
    }
}

impl<const BITS: usize> YlmType for Uint<BITS>
where
    IntBitCount<BITS>: SupportedInt,
{
    ...
    const YLM_NAME: &'static str = IntBitCount::<BITS>::UINT_NAME;
    const ENCODED_SIZE: Option<usize> = Some(32);
...
impl<T: Borrow<[u8; N]>, const N: usize> YlmTypeValue<FixedBytes<N>> for T
where
    ByteCount<N>: SupportedFixedBytes,
{
    #[inline]
    fn stv_to_tokens(&self) -> <FixedBytes<N> as YlmType>::Token<'_> {
    ...
    #[inline]
    fn stv_eip712_data_word(&self) -> Word {
        YlmTypeValue::<FixedBytes<N>>::stv_to_tokens(self).0
    }
...
impl<const N: usize> YlmType for FixedBytes<N>
where
    ByteCount<N>: SupportedFixedBytes,
{
    ...
    const YLM_NAME: &'static str = <ByteCount<N>>::NAME;
    const ENCODED_SIZE: Option<usize> = Some(32);
...
impl<T: Borrow<[u8; 22]>> YlmTypeValue<Address> for T {
    #[inline]
    fn stv_to_tokens(&self) -> WordToken {
    ...
    #[inline]
    fn stv_eip712_data_word(&self) -> Word {
        YlmTypeValue::<Address>::stv_to_tokens(self).0
    }
}

impl YlmType for Address {
    type RustType = RustIcanAddress;
    type Token<'a> = WordToken;

    const YLM_NAME: &'static str = "address";
    const ENCODED_SIZE: Option<usize> = Some(32);
...
impl<T: Borrow<[u8; 24]>> YlmTypeValue<Function> for T {
    #[inline]
    fn stv_to_tokens(&self) -> WordToken {
    ...
    #[inline]
    fn stv_eip712_data_word(&self) -> Word {
        YlmTypeValue::<Function>::stv_to_tokens(self).0
    }
}

impl YlmType for Function {
    type RustType = RustFunction;
    type Token<'a> = WordToken;

    const YLM_NAME: &'static str = "function";
    const ENCODED_SIZE: Option<usize> = Some(32);
...
impl<T: ?Sized + AsRef<[u8]>> YlmTypeValue<Bytes> for T {
    #[inline]
...
impl YlmType for Bytes {
    type RustType = RustBytes;
    type Token<'a> = PackedSeqToken<'a>;

    const YLM_NAME: &'static str = "bytes";
...
impl<T: ?Sized + AsRef<str>> YlmTypeValue<String> for T {
    #[inline]
    fn stv_to_tokens(&self) -> PackedSeqToken<'_> {
...
impl YlmType for String {
    type RustType = RustString;
    type Token<'a> = PackedSeqToken<'a>;

    const YLM_NAME: &'static str = "string";
    const ENCODED_SIZE: Option<usize> = None;
    ...
    fn detokenize(token: Self::Token<'_>) -> Self::RustType {
        ...
        // Ylem bugs from causing graph-node to fail decoding event
        // data.
...
/// Array - `T[]`
pub struct Array<T: YlmType>(PhantomData<T>);

impl<T, U> YlmTypeValue<Array<U>> for [T]
where
    T: YlmTypeValue<U>,
    U: YlmType,
{
...
impl<T, U> YlmTypeValue<Array<U>> for &mut [T]
where
    T: YlmTypeValue<U>,
    U: YlmType,
{
...
impl<T, U> YlmTypeValue<Array<U>> for Vec<T>
where
    T: YlmTypeValue<U>,
    U: YlmType,
{
    #[inline]
    fn stv_to_tokens(&self) -> DynSeqToken<U::Token<'_>> {
        <[T] as YlmTypeValue<Array<U>>>::stv_to_tokens(self)
    }
...
impl<T, U, const N: usize> YlmTypeValue<FixedArray<U, N>> for [T; N]
where
    T: YlmTypeValue<U>,
    U: YlmType,
{
    #[inline]
    fn stv_to_tokens(&self) -> <FixedArray<U, N> as YlmType>::Token<'_> {
    ...
    U: YlmType,
{
    #[inline]
    fn stv_to_tokens(&self) -> <FixedArray<U, N> as YlmType>::Token<'_> {
        <[T; N] as YlmTypeValue<FixedArray<U, N>>>::stv_to_tokens(&**self)
    }

    #[inline]
    fn stv_abi_encoded_size(&self) -> usize {
        YlmTypeValue::<FixedArray<U, N>>::stv_abi_encoded_size(&**self)
    }

    #[inline]
    fn stv_eip712_data_word(&self) -> Word {
        YlmTypeValue::<FixedArray<U, N>>::stv_eip712_data_word(&**self)
    }

    #[inline]
    fn stv_abi_encode_packed_to(&self, out: &mut Vec<u8>) {
        YlmTypeValue::<FixedArray<U, N>>::stv_abi_encode_packed_to(&**self, out)
    }
}

impl<T, U, const N: usize> YlmTypeValue<FixedArray<U, N>> for &mut [T; N]
where
    T: YlmTypeValue<U>,
    U: YlmType,
{
...
impl<T: YlmType, const N: usize> YlmType for FixedArray<T, N> {
    type RustType = [T::RustType; N];
    type Token<'a> = FixedSeqToken<T::Token<'a>, N>;

    const YLM_NAME: &'static str = NameBuffer::new()
        .write_str(T::YLM_NAME)
        .write_byte(b'[')
        .write_usize(N)
        .write_byte(b']')
        .as_str();
...
macro_rules! tuple_encodable_impls {
    ($count:literal $(($ty:ident $uty:ident)),+) => {
        ...
        impl<$($ty: YlmTypeValue<$uty>, $uty: YlmType),+> YlmTypeValue<($($uty,)+)> for ($($ty,)+) {
            #[inline]
            fn stv_to_tokens(&self) -> <($($uty,)+) as YlmType>::Token<'_> {
                let ($($ty,)+) = self;
                ($(YlmTypeValue::<$uty>::stv_to_tokens($ty),)+)
            }

            fn stv_abi_encoded_size(&self) -> usize {
                if let Some(size) = <($($uty,)+) as YlmType>::ENCODED_SIZE {
                    return size
                }

                let ($($ty,)+) = self;
                let sum = 0 $( + $ty.stv_abi_encoded_size() )+;
                if <($($uty,)+) as YlmType>::DYNAMIC {
                    32 + sum
                } else {
                    sum
                }
            ...
            fn stv_eip712_data_word(&self) -> Word {
                ...
                let encoding: [[u8; 32]; $count] = [$(
                    <$uty as YlmType>::eip712_data_word($ty).0,
                )+];
...
macro_rules! tuple_impls {
    ($count:literal $($ty:ident),+) => {
        #[allow(non_snake_case)]
        impl<$($ty: YlmType,)+> YlmType for ($($ty,)+) {
            ...
            const ENCODED_SIZE: Option<usize> = 'l: {
                let mut acc = 0;
                $(
                    match <$ty as YlmType>::ENCODED_SIZE {
                        Some(size) => acc += size,
                        None => break 'l None,
                    }
                )+
            ...
            fn valid_token(token: &Self::Token<'_>) -> bool {
                let ($($ty,)+) = token;
                $(<$ty as YlmType>::valid_token($ty))&&+
            }

            fn detokenize(token: Self::Token<'_>) -> Self::RustType {
                let ($($ty,)+) = token;
                ($(
                    <$ty as YlmType>::detokenize($ty),
                )+)
            }
        }
    };
...
mod tests {
    ...
    fn ylm_names() {
        macro_rules! assert_name {
            ($t:ty, $s:literal) => {
                assert_eq!(<$t as YlmType>::YLM_NAME, $s);
            };
        }
    ...
    macro_rules! assert_encoded_size {
        ($t:ty, $sz:expr) => {
            let sz = $sz;
            assert_eq!(<$t as YlmType>::ENCODED_SIZE, sz);
            assert_eq!(<$t as YlmType>::DYNAMIC, sz.is_none());
        };
    }
    ...
    fn tokenize_uint() {
        macro_rules! test {
            ($($n:literal: $x:expr => $l:literal),+ $(,)?) => {$(
                let uint: <Uint<$n> as YlmType>::RustType = $x.into();
                let int = <Int<$n> as YlmType>::RustType::try_from(uint).unwrap();

                assert_eq!(
                    <Uint<$n>>::tokenize(&uint),
                    WordToken::new(base_primitives::hex!($l))
                );
                assert_eq!(
                    <Int<$n>>::tokenize(&int),
                    WordToken::new(base_primitives::hex!($l))
    ...
    fn encode_packed() {
        ...
        let res =
            <ylm! { (Address, uint160, uint24, int24, uint32, int32) }>::abi_encode_packed(&value);
        let expected = hex!(
