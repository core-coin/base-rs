use base_primitives::{hex, sha3, U256};
use base_ylm_types::{ylm, YlmCall, YlmError};
...
ylm! {
    ...
    // State variables will generate getter functions just like in Ylem.
...
fn assert_call_signature<T: YlmCall>(expected: &str) {
    assert_eq!(T::SIGNATURE, expected);
    assert_eq!(T::SELECTOR, sha3(expected)[..4]);
}

fn assert_error_signature<T: YlmError>(expected: &str) {
    assert_eq!(T::SIGNATURE, expected);
    assert_eq!(T::SELECTOR, sha3(expected)[..4]);
}
