...
///
/// ```
/// # use base_ylm_type_parser::RootType;
...
/// # Ok::<_, base_ylm_type_parser::Error>(())
/// ```
...
impl<'a> RootType<'a> {
    ...
    /// # Safety
    ///
    /// The string passed in must be a valid Ylem identifier. See
    /// [`is_valid_identifier`].
    ...
    pub fn parser(input: &mut &'a str) -> PResult<Self> {
        trace("RootType", |input: &mut &'a str| {
            identifier(input).map(|ident| {
                ...
                // See: https://github.com/core-coin/base-rs/pull/386
    ...
    /// Returns `Ok(())` if the type is a basic Ylem type.
    #[inline]
