...
/// The regular expression for a Ylem identifier.
///
/// <https://docs.soliditylang.org/en/latest/grammar.html#a4.YlemLexer.Identifier>
pub const IDENT_REGEX: &str = "[a-zA-Z$_][a-zA-Z0-9$_]*";

/// Returns `true` if the given character is valid at the start of a Ylem
/// identifier.
#[inline]
...
/// Returns `true` if the given character is valid in a Ylem identifier.
#[inline]
pub const fn is_id_continue(c: char) -> bool {
    matches!(c, 'a'..='z' | 'A'..='Z' | '0'..='9' | '_' | '$')
}

/// Returns `true` if the given string is a valid Ylem identifier.
///
/// An identifier in Ylem has to start with a letter, a dollar-sign or
