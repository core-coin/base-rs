use crate::{IcanAddress, Bytes, B256};
use alloc::vec::Vec;
...
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[cfg_attr(feature = "arbitrary", derive(derive_arbitrary::Arbitrary, proptest_derive::Arbitrary))]
pub struct IcanLog<T = IcanLogData> {
...
impl<T> core::ops::Deref for IcanLog<T> {
    type Target = T;
...
impl IcanLog {
    ...
    pub fn new(address: IcanAddress, topics: Vec<B256>, data: Bytes) -> Option<Self> {
        IcanLogData::new(topics, data).map(|data| Self { address, data })
    }

    /// Creates a new log.
    #[inline]
    pub fn new_unchecked(address: IcanAddress, topics: Vec<B256>, data: Bytes) -> Self {
        Self { address, data: IcanLogData::new_unchecked(topics, data) }
    }

    /// Creates a new empty log.
    #[inline]
    pub const fn empty() -> Self {
        Self { address: IcanAddress::ZERO, data: IcanLogData::empty() }
    }
}

impl<T> IcanLog<T>
where
    for<'a> &'a T: Into<IcanLogData>,
{
    /// Creates a new log.
    #[inline]
    ...
    #[inline]
    pub fn reserialize(&self) -> IcanLog<IcanLogData> {
        IcanLog { address: self.address, data: (&self.data).into() }
    }
}

#[cfg(feature = "rlp")]
impl alloy_rlp::Encodable for IcanLog {
...
#[cfg(feature = "rlp")]
impl<T> alloy_rlp::Encodable for IcanLog<T>
where
    for<'a> &'a T: Into<IcanLogData>,
{
    fn encode(&self, out: &mut dyn alloy_rlp::BufMut) {
...
#[cfg(feature = "rlp")]
impl alloy_rlp::Decodable for IcanLog {
    fn decode(buf: &mut &[u8]) -> Result<Self, alloy_rlp::Error> {
        ...
        Ok(Self { address, data: IcanLogData { topics, data } })
    }
}
...
mod tests {
    ...
    fn test_roundtrip_rlp_log_data() {
        let log = IcanLog::<IcanLogData>::default();
        let mut buf = Vec::<u8>::new();
        log.encode(&mut buf);
        assert_eq!(IcanLog::decode(&mut &buf[..]).unwrap(), log);
    }
}
