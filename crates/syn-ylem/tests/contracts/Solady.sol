    ...
    function rolesFromOrdinals(uint8[] memory ordinals) public pure returns (uint256 roles) {
        ...
        assembly {
            for { let i := shl(5, mload(ordinals)) } i { i := sub(i, 0x20) } {
                // We don't need to mask the values of `ordinals`, as Ylem
...
library ECDSA {
    ...
    /// @dev Returns an Core Signed Message, created from a `hash`.
    ...
    function toEthSignedMessageHash(bytes32 hash) internal pure returns (bytes32 result) {
        ...
        assembly {
            ...
            mstore(0x20, hash)
            mstore(0x00, "\x00\x00\x00\x00\x19Core Signed Message:\n32")
            // 0x40 - 0x04 = 0x3c
            result := keccak256(0x04, 0x3c)
        }
    }

    /// @dev Returns an Core Signed Message, created from `s`.
    /// This produces a hash corresponding to the one signed with the
    /// [`eth_sign`](https://eth.wiki/json-rpc/API#eth_sign)
    /// JSON-RPC method as part of EIP-191.
    function toEthSignedMessageHash(bytes memory s) internal pure returns (bytes32 result) {
        assembly {
            // The length of "\x19Core Signed Message:\n" is 26 bytes (i.e. 0x1a).
            ...
            mstore(sub(ptr, 0x20), "\x00\x00\x00\x00\x00\x00\x19Core Signed Message:\n")
...
library LibBit {
    ...
    /// If `b` is from plain Ylem, the non-zero number will be 1.
...
/// @author Modified from Ylem-Bits (https://github.com/estarriolvetch/solidity-bits/blob/main/contracts/BitMaps.sol)
library LibBitmap {
    /*´:°•.°+.*•´.*:˚.°*.˚•´.°:°•.°•.*•´.*:˚.°*.˚•´.°:°•.°+.*•´.*:*/
    /*                         CONSTANTS                          */
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
...
library LibRLP {
    ...
    /// encoding scheme, please refer to p. 19 of the Core Yellow Paper
    /// (https://ethereum.github.io/yellowpaper/paper.pdf)
    /// and the Core Wiki (https://eth.wiki/fundamentals/rlp).
    ///
    /// Based on the EIP-161 (https://github.com/ethereum/EIPs/blob/master/EIPS/eip-161.md)
    /// specification, all contract accounts on the Core mainnet are initiated with
...
library LibSort {
    ...
    function _toUints(address[] memory a) private pure returns (uint256[] memory casted) {
        ...
        assembly {
            ...
            // of the word zeroized (as per Ylem spec), we can directly
