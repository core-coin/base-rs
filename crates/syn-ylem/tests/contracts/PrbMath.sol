...
function gm(UD60x18 x, UD60x18 y) pure returns (UD60x18 result) {
    ...
    unchecked {
        // Checking for overflow this way is faster than letting Ylem do it.
...
/// decimals. The values of this are bound by the minimum and the maximum values permitted by the Ylem type uint256.
...
/// decimals. The values of this are bound by the minimum and the maximum values permitted by the underlying Ylem
...
function ceil(SD59x18 x) pure returns (SD59x18 result) {
    ...
    if (remainder == 0) {
        ...
        unchecked {
            // Ylem uses C fmod style, which returns a modulus with the same sign as x.
...
function gm(SD59x18 x, SD59x18 y) pure returns (SD59x18 result) {
    ...
    unchecked {
        // Equivalent to `xy / x != y`. Checking for overflow this way is faster than letting Ylem do it.
...
interface VmSafe {
    ...
    /// encodes the fields in the same order, which is alphabetical. In the case of Ylem structs, they are encoded
...
interface Vm is VmSafe {
    ...
    /// Calldata can either be strict or a partial match, e.g. if you only pass a Ylem selector to the expected
    /// calldata, then the entire Ylem function will be mocked.
