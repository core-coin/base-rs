...
use base_ylm_macro_input::{derives_mapped, mk_doc, ContainsYlmAttrs};
...
pub(super) fn expand(cx: &ExpCtxt<'_>, enumm: &ItemEnum) -> Result<TokenStream> {
    ...
    let (ylm_attrs, mut attrs) = enumm.split_attrs()?;
    ...
    let uint8 = quote!(base_ylm_types::ylm_data::Uint<8>);
    let uint8_st = quote!(<#uint8 as base_ylm_types::YlmType>);
    ...
    let tokens = quote! {
        ...
        const _: () = {
            ...
            impl ::core::convert::TryFrom<u8> for #name {
                ...
                #[allow(unsafe_code)]
                #[inline]
                fn try_from(v: u8) -> base_ylm_types::Result<Self> {
                    if v <= #max {
                        ::core::result::Result::Ok(unsafe { ::core::mem::transmute(v) })
                    } else {
                        ::core::result::Result::Err(base_ylm_types::Error::InvalidEnumValue {
                            name: #name_s,
                            value: v,
                            max: #max,
                        })
            ...
            #[automatically_derived]
            impl base_ylm_types::private::YlmTypeValue<#name> for #name {
                #[inline]
                fn stv_to_tokens(&self) -> #uint8_st::Token<'_> {
                    base_ylm_types::Word::with_last_byte(*self as u8).into()
                }
            ...
            impl base_ylm_types::YlmType for #name {
                ...
                const YLM_NAME: &'static str = #uint8_st::YLM_NAME;
            ...
            impl base_ylm_types::EventTopic for #name {
                ...
                fn topic_preimage_length(rust: &Self::RustType) -> usize {
                    <#uint8 as base_ylm_types::EventTopic>::topic_preimage_length(rust.as_u8())
                }

                #[inline]
                fn encode_topic_preimage(rust: &Self::RustType, out: &mut base_ylm_types::private::Vec<u8>) {
                    <#uint8 as base_ylm_types::EventTopic>::encode_topic_preimage(rust.as_u8(), out);
                }

                #[inline]
                fn encode_topic(rust: &Self::RustType) -> base_ylm_types::abi::token::WordToken {
                    <#uint8 as base_ylm_types::EventTopic>::encode_topic(rust.as_u8())
                }
            }

            #[automatically_derived]
            impl base_ylm_types::YlmEnum for #name {
