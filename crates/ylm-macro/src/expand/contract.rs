...
use ast::{Item, ItemContract, ItemError, ItemEvent, ItemFunction, Spanned, YlmIdent};
use base_ylm_macro_input::{docs_str, mk_doc, ContainsYlmAttrs};
...
pub(super) fn expand(cx: &ExpCtxt<'_>, contract: &ItemContract) -> Result<TokenStream> {
    ...
    let bytecode = ylm_attrs.bytecode.map(|lit| {
        ...
        quote! {
            ...
            /// ```
            #[rustfmt::skip]
            pub static #name: base_ylm_types::private::Bytes =
                base_ylm_types::private::Bytes::from_static(#lit_bytes);
        }
    });
    let deployed_bytecode = ylm_attrs.deployed_bytecode.map(|lit| {
    ...
    let abi = abi.then(|| {
        if_json! {
            ...
            quote! {
                /// Contains [dynamic ABI definitions](base_ylm_types::private::base_json_abi) for [this contract](self).
                pub mod abi {
                    use super::*;
                    use base_ylm_types::private::{base_json_abi as json, BTreeMap, Vec};
    ...
    let rpc = rpc.then(|| {
        ...
        let struct_doc = format!(
            "A [`{contract_name}`](self) instance.\n\
             ...
             provider `P`.\n\
             \n\
             If the contract bytecode is available (see the [`ylm!`](base_ylm_types::ylm!)\n\
        ...
        let (deploy_fn, deploy_method) = option_unzip(bytecode.is_some().then(|| {
            ...
            let deploy_builder_data = if matches!(constructor, Some(c) if !c.parameters.is_empty()) {
                quote! {
                    [
                        &BYTECODE[..],
                        &base_ylm_types::YlmConstructor::abi_encode(&constructorCall { #args })[..]
                    ].concat().into()
                }
            } else {
                quote! {
            ...
            (
                quote! {
                    #deploy_doc
                    #[inline]
                    pub fn deploy<T: base_contract::private::Transport + ::core::clone::Clone, P: base_contract::private::Provider<T, N>, N: base_contract::private::Network>(provider: P, #params)
                        -> impl ::core::future::Future<Output = base_contract::Result<#name<T, P, N>>>
                    {
                    ...
                    #deploy_builder_doc
                    #[inline]
                    pub fn deploy_builder<T: base_contract::private::Transport + ::core::clone::Clone, P: base_contract::private::Provider<T, N>, N: base_contract::private::Network>(provider: P, #params)
                ...
                quote! {
                    ...
                    pub async fn deploy(provider: P, #params)
                        -> base_contract::Result<#name<T, P, N>>
                    {
        ...
        let filter_methods = events.iter().map(|&e| {
            ...
            quote! {
                #[doc = #doc]
                pub fn #name(&self) -> base_contract::Event<T, &P, #event_name, N> {
        ...
        quote! {
            ...
            impl #generics_t_p_n #name<T, P, N> {
                #[doc = #new_fn_doc]
                #[inline]
                pub const fn new(address: base_ylm_types::private::Address, provider: P) -> Self {
                ...
                #[inline]
                pub const fn address(&self) -> &base_ylm_types::private::Address {
                    &self.address
                }

                /// Sets the address.
                #[inline]
                pub fn set_address(&mut self, address: base_ylm_types::private::Address) {
                    self.address = address;
                }

                /// Sets the address and returns `self`.
                pub fn at(mut self, address: base_ylm_types::private::Address) -> Self {
            ...
            impl #generics_t_p_n #name<T, P, N> {
                ...
                pub fn call_builder<C: base_ylm_types::YlmCall>(&self, call: &C)
                    -> base_contract::YlmCallBuilder<T, &P, C, N>
                {
                    base_contract::YlmCallBuilder::new_sol(&self.provider, &self.address, call)
                }
            ...
            impl #generics_t_p_n #name<T, P, N> {
                ...
                pub fn event_filter<E: base_ylm_types::YlmEvent>(&self)
                    -> base_contract::Event<T, &P, E, N>
                {
                    base_contract::Event::new_sol(&self.provider, &self.address)
                }
...
impl<'a> CallLikeExpander<'a> {
    fn expand(&self, to_expand: ToExpand<'_>, attrs: Vec<Attribute>) -> TokenStream {
        ...
        quote! {
            ...
            #[automatically_derived]
            impl base_ylm_types::YlmInterface for #name {
                ...
                fn selector(&self) -> [u8; 4] {
                    match self {#(
                        Self::#variants(_) => <#types as base_ylm_types::#trait_>::SELECTOR,
                    )*}
                }
                ...
                fn abi_decode_raw(
                    ...
                    data: &[u8],
                    validate: bool
                )-> base_ylm_types::Result<Self> {
                    static DECODE_SHIMS: &[fn(&[u8], bool) -> base_ylm_types::Result<#name>] = &[
                        #({
                            fn #sorted_variants(data: &[u8], validate: bool) -> base_ylm_types::Result<#name> {
                                <#sorted_types as base_ylm_types::#trait_>::abi_decode_raw(data, validate)
                    ...
                    let Ok(idx) = Self::SELECTORS.binary_search(&selector) else {
                        return Err(base_ylm_types::Error::unknown_selector(
                            <Self as base_ylm_types::YlmInterface>::NAME,
                            selector,
                        ));
                    };
                ...
                fn abi_encoded_size(&self) -> usize {
                    match self {#(
                        Self::#variants(inner) =>
                            <#types as base_ylm_types::#trait_>::abi_encoded_size(inner),
                    )*}
                }

                #[inline]
                fn abi_encode_raw(&self, out: &mut base_ylm_types::private::Vec<u8>) {
                    match self {#(
                        Self::#variants(inner) =>
                            <#types as base_ylm_types::#trait_>::abi_encode_raw(inner, out),
                    )*}
                }
            }
        }
    ...
    fn expand_events(
        ...
        let err = quote! {
            base_ylm_types::private::Err(base_ylm_types::Error::InvalidLog {
                name: <Self as base_ylm_types::YlmEventInterface>::NAME,
                log: base_ylm_types::private::Box::new(base_ylm_types::private::LogData::new_unchecked(
                    topics.to_vec(),
                    data.to_vec().into(),
                )),
            })
        ...
        let non_anon_impl = has_non_anon.then(|| {
            ...
            quote! {
                match topics.first().copied() {
                    #(
                        Some(<#variants as base_ylm_types::#trait_>::SIGNATURE_HASH) =>
                            #ret <#variants as base_ylm_types::#trait_>::decode_raw_log(topics, data, validate)
                                .map(Self::#variants),
                    )*
                    _ => { #ret_err }
                }
        ...
        let anon_impl = has_anon.then(|| {
            ...
            quote! {
                #(
                    if let Ok(res) = <#variants as base_ylm_types::#trait_>::decode_raw_log(topics, data, validate) {
        ...
        quote! {
            ...
            impl base_ylm_types::YlmEventInterface for #name {
                const NAME: &'static str = #name_s;
                const COUNT: usize = #count;

                fn decode_raw_log(topics: &[base_ylm_types::Word], data: &[u8], validate: bool) -> base_ylm_types::Result<Self> {
                    #non_anon_impl
                    #anon_impl
                }
            }
    ...
    fn generate_enum(
        ...
        let mut tokens = quote! {
            ...
            impl #name {
                ...
                ///
                /// Prefer using `YlmInterface` methods instead.
                // NOTE: This is currently sorted to allow for binary search in `YlmInterface`.
...
fn call_builder_method(f: &ItemFunction, cx: &ExpCtxt<'_>) -> TokenStream {
    ...
    quote! {
        #[doc = #doc]
        pub fn #name(&self, #(#param_names1: #param_tys),*) -> base_contract::YlmCallBuilder<T, &P, #call_name, N> {
