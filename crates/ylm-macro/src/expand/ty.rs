...
/// Expands a single [`Type`] recursively to its `base_ylm_types::ylm_data`
/// equivalent.
...
///
/// This is the same as `<#expand_type(ty) as YlmType>::RustType`, but generates
...
pub(crate) fn rec_expand_type(ty: &Type, crates: &ExternCrates, tokens: &mut TokenStream) {
    let base_ylm_types = &crates.ylm_types;
    let tts = match *ty {
        Type::Address(span, _) => quote_spanned! {span=> #base_ylm_types::ylm_data::Address },
        Type::Bool(span) => quote_spanned! {span=> #base_ylm_types::ylm_data::Bool },
        Type::String(span) => quote_spanned! {span=> #base_ylm_types::ylm_data::String },
        Type::Bytes(span) => quote_spanned! {span=> #base_ylm_types::ylm_data::Bytes },

        Type::FixedBytes(span, size) => {
            assert!(size.get() <= 32);
            let size = Literal::u16_unsuffixed(size.get());
            quote_spanned! {span=> #base_ylm_types::ylm_data::FixedBytes<#size> }
        }
        Type::Int(span, size) | Type::Uint(span, size) => {
            ...
            quote_spanned! {span=> #base_ylm_types::ylm_data::#name<#size> }
        }
        ...
        Type::Array(ref array) => {
            ...
            if let Some(size) = array.size() {
                quote_spanned! {span=> #base_ylm_types::ylm_data::FixedArray<#ty, #size> }
            } else {
                quote_spanned! {span=> #base_ylm_types::ylm_data::Array<#ty> }
            }
        }
        Type::Function(ref function) => quote_spanned! {function.span()=>
            #base_ylm_types::ylm_data::Function
        },
...
// IMPORTANT: Keep in sync with `ylm-types/src/types/data_type.rs`
...
pub(crate) fn rec_expand_rust_type(ty: &Type, crates: &ExternCrates, tokens: &mut TokenStream) {
    ...
    let base_ylm_types = &crates.ylm_types;
    let tts = match *ty {
        Type::Address(span, _) => quote_spanned! {span=> #base_ylm_types::private::Address },
        Type::Bool(span) => return Ident::new("bool", span).to_tokens(tokens),
        Type::String(span) => quote_spanned! {span=> #base_ylm_types::private::String },
        Type::Bytes(span) => quote_spanned! {span=> #base_ylm_types::private::Bytes },

        Type::FixedBytes(span, size) => {
            assert!(size.get() <= 32);
            let size = Literal::u16_unsuffixed(size.get());
            quote_spanned! {span=> #base_ylm_types::private::FixedBytes<#size> }
        }
        Type::Int(span, size) | Type::Uint(span, size) if allowed_int_size(size) => {
            ...
            assert_eq!(size, 256);
            match ty {
                Type::Int(..) => quote_spanned! {span=> #base_ylm_types::private::I256 },
                Type::Uint(..) => quote_spanned! {span=> #base_ylm_types::private::U256 },
                _ => unreachable!(),
            }
        }
        ...
        Type::Array(ref array) => {
            ...
            if let Some(size) = array.size() {
                ...
                quote_spanned! {span=> #base_ylm_types::private::Vec<#ty> }
            }
        }
        Type::Function(ref function) => quote_spanned! {function.span()=>
            #base_ylm_types::private::Function
        },
        Type::Mapping(ref mapping) => quote_spanned! {mapping.span()=>
            ::core::compile_error!("Mapping types are not supported here")
        },

        // Exhaustive fallback to `YlmType::RustType`
        ref ty @ (Type::Int(..) | Type::Uint(..) | Type::Custom(_)) => {
            let span = ty.span();
            let ty = expand_type(ty, crates);
            quote_spanned! {span=> <#ty as #base_ylm_types::YlmType>::RustType }
        }
    };
    tokens.extend(tts);
}
