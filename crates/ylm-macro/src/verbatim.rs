...
impl Verbatim for String {
    fn to_verbatim_tokens(&self, tokens: &mut TokenStream, crates: &ExternCrates) {
        ...
        tokens.extend(if self.is_empty() {
            quote!(#base_ylm_types::private::String::new())
        } else {
            quote!(#base_ylm_types::private::ToOwned::to_owned(#self))
        })
    }
}
...
impl<T: Verbatim> Verbatim for Vec<T> {
    fn to_verbatim_tokens(&self, s: &mut TokenStream, crates: &ExternCrates) {
        ...
        s.extend(if self.is_empty() {
            ...
            quote!(#base_ylm_types::private::vec![#(#iter),*])
        });
    }
}
...
macro_rules! derive_verbatim {
    ...
    (struct $name:ident { $($field:ident),* $(,)? } $($rest:tt)*) => {
        impl Verbatim for base_json_abi::$name {
            fn to_verbatim_tokens(&self, s: &mut TokenStream, crates: &ExternCrates) {
                let Self { $($field),* } = self;
                $(
                ...
                let base_ylm_types = &crates.ylm_types;
                s.extend(quote! {
                    #base_ylm_types::private::base_json_abi::$name {
                        $($field: #$field,)*
                    }
                });
            }
            ...
            fn to_verbatim_tokens(&self, s: &mut TokenStream, crates: &ExternCrates) {
                match self {$(
                    Self::$variant $( { $($field_idx: $field),* } )? => {
                        ...
                        let base_ylm_types = &crates.ylm_types;
                        s.extend(quote! {
                            #base_ylm_types::private::base_json_abi::$name::$variant $( { $($field_idx: #$field),* } )?
                        });
                    }
                )*}
            }
