...
/// Configuration for [`JsonAbi::to_ylm`].
#[derive(Clone, Debug)]
#[allow(missing_copy_implementations)] // Future-proofing
pub struct ToYlmConfig {
    print_constructors: bool,
}

impl Default for ToYlmConfig {
    #[inline]
    fn default() -> Self {
        Self::new()
    }
...
pub(crate) trait ToYlm {
    fn to_ylm(&self, out: &mut YlmPrinter<'_>);
}

pub(crate) struct YlmPrinter<'a> {
    /// The buffer to write to.
    s: &'a mut String,

    /// Whether to emit `memory` when printing parameters.
    /// This is set to `true` when printing functions so that we emit valid Ylem.
...
impl DerefMut for YlmPrinter<'_> {
    #[inline]
...
impl<'a> YlmPrinter<'a> {
    #[inline]
    pub(crate) fn new(s: &'a mut String, config: ToYlmConfig) -> Self {
        Self { s, emit_param_location: false, config }
    }

    #[inline]
    pub(crate) fn print<T: ToYlm>(&mut self, value: &T) {
        value.to_ylm(self);
    }
    ...
    fn extend(
        ...
        match internal_type {
            ...
            Some(it @ InternalType::Other { contract: _, ty }) => {
                // `Other` is a UDVT if it's not a basic Ylem type and not an array
...
impl<IN: ToYlm> ToYlm for AbiFunction<'_, IN> {
    fn to_ylm(&self, out: &mut YlmPrinter<'_>) {
        if matches!(
            self.kw,
...
fn param(
    ...
    match type_name.strip_prefix("tuple") {
        ...
        // See https://github.com/core-coin/base-rs/issues/349
        Some(rest) if rest.is_empty() || rest.starts_with('[') => {
            // note: this does not actually emit valid Ylem because there are no inline
            // tuple types `(T, U, V, ...)`, but it's valid for `ylm!`.
            out.push('(');
            // Don't emit `memory` for tuple components because `ylm!` can't parse them.
