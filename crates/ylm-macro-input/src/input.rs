...
/// Parsed input for `ylm!`-like macro expanders. This enum represents a `Sol` file, a JSON ABI, or
/// a Ylem type.
#[derive(Clone, Debug)]
pub enum YlmInputKind {
    /// Ylem type.
    Type(ast::Type),
    /// Ylem file or snippet.
    Sol(ast::File),
    /// JSON ABI file
    #[cfg(feature = "json")]
    Json(Ident, base_json_abi::ContractObject),
}

impl Parse for YlmInputKind {
...
/// Parsed input for `ylm!`-like macro expanders. This struct represents a list
...
pub struct YlmInput {
    ...
    /// The input kind.
    pub kind: YlmInputKind,
}

impl Parse for YlmInput {
    fn parse(input: ParseStream<'_>) -> Result<Self> {
...
impl YlmInput {
    /// `abigen`-like syntax: `ylm!(name, "path/to/file")`
    fn parse_abigen(mut attrs: Vec<Attribute>, input: ParseStream<'_>) -> Result<Self> {
            ...
            || (s.starts_with('[') && s.ends_with(']'))
        {
            #[cfg(feature = "json")]
            {
                ...
                Ok(Self { attrs, path, kind: YlmInputKind::Json(name, json) })
            }
            ...
            let kind = syn::parse_str(s).map_err(|e| {
                let msg = format!("expected a valid JSON ABI string or Ylem string: {e}");
                Error::new(span, msg)
            })?;
