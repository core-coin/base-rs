use crate::{YlmInput, YlmInputKind};
use base_json_abi::{ContractObject, JsonAbi, ToYlmConfig};
...
impl YlmInput {
    ...
    pub fn normalize_json(self) -> Result<Self> {
        let YlmInput {
            attrs,
            path,
            kind: YlmInputKind::Json(name, ContractObject { abi, bytecode, deployed_bytecode }),
        } = self
        else {
            return Ok(self);
        };
        ...
        let ylm = abi_to_ylm(&name, &mut abi);
        let ylm_interface_tokens = tokens_for_sol(&name, &ylm)?;
        ...
        let doc_str = format!(
            "\n\n\
Generated by the following Ylem interface...
```solidity
{ylm}
```
...
```",
        ...
        let tokens = quote! {
            #(#attrs_iter)*
            #[doc = #doc_str]
            #[ylm(#bytecode #deployed_bytecode)]
            #ylm_interface_tokens
        };

        let ast: ast::File = syn::parse2(tokens).map_err(|e| {
            let msg = format!(
                "failed to parse ABI-generated tokens into a Ylem AST: {e}.\n\
        ...
        let kind = YlmInputKind::Sol(ast);
        Ok(YlmInput { attrs, path, kind })
    }
}

// doesn't parse Json

fn abi_to_ylm(name: &Ident, abi: &mut JsonAbi) -> String {
    abi.dedup();
    abi.to_ylm(&name.to_string(), Some(ToYlmConfig::new().print_constructors(true)))
}
...
fn tokens_for_sol(name: &Ident, sol: &str) -> Result<TokenStream> {
    let mk_err = |s: &str| {
        let msg = format!(
            "`JsonAbi::to_ylm` generated invalid Rust tokens: {s}\n\
             This is a bug. We would appreciate a bug report: \
             https://github.com/core-coin/base-rs/issues/new/choose"
        );
        syn::Error::new(name.span(), msg)
...
mod tests {
    ...
    fn expand_test<'a>(s: &str, path: &'a str) -> (ast::ItemContract, &'a str) {
        ...
        let name_id = id(name);
        let sol = abi_to_ylm(&name_id, &mut abi);
    ...
    fn write_tmp_sol(name: &str, contents: &str) -> PathBuf {
        let path = std::env::temp_dir().join(format!("ylm-macro-{name}.sol"));
